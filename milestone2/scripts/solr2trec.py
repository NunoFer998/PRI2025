...
def solr_to_trec(solr_response, run_id="run0"):
    ...
    # --- THIS IS THE FIX ---
    # Loop through the dictionary, where 'query_id' is the key (e.g., "1", "2")
    # and 'response' is the value (the Solr JSON response)
    for query_id, response in solr_response.items():
        try:
            ...
            for rank, doc in enumerate(docs, start=1):
                # Use the 'query_id' from the loop, not a hardcoded "0"
                print(f"{int(query_id)} Q0 {doc['id']} {rank} {doc['score']} {run_id}")

        except KeyError:
            print(f"Error: Invalid Solr response format for query {query_id}. 'docs' key not found.", file=sys.stderr)
            # Don't exit; try to process the next query
            continue
