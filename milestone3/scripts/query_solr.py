...
def query_solr(query_text, system_file, solr_uri="http://localhost:8983/solr", collection="diseases", rows=10):
    """
    Query Solr with a text query and system configuration.

    Arguments:
    - query_text: The search query string.
    - system_file: Path object to the JSON file containing system parameters.
    - solr_uri: URI of the Solr instance (default: http://localhost:8983/solr).
    - collection: Solr collection name (default: diseases).
    - rows: Number of results to return (default: 10).

    Returns:
    - Dictionary containing the Solr response, or raises an exception on error.
    """
    # Load the system parameters from the JSON file
    try:
        system_params = json.loads(system_file.read_text())
    except FileNotFoundError:
        raise FileNotFoundError(f"System file not found: {system_file}")
    except json.JSONDecodeError as e:
        raise ValueError(f"Invalid JSON format in system file: {e}")

    # Construct the Solr request URL
    uri = f"{solr_uri}/{collection}/select"

    # Build params with query and system configuration
    params = {
        **system_params,
        "q": query_text,
        "rows": rows,
    ...
    try:
        ...
        raise ConnectionError(f"Error querying Solr: {e}")
